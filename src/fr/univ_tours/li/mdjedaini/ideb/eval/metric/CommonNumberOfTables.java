...
public class CommonNumberOfTables extends Metric{

	public CommonNumberOfTables(BenchmarkEngine arg_be) {
        super(arg_be);
        this.name           = "Metric - Common tables number";
        this.description    = "Computes iteratively the number of common tables between queries";
    }
    ...
    public MetricScore apply(Exploration arg_tr) {
        ...
        for(int i = 0; i < arg_tr.getWorkSession().getNumberOfQueries(); i++) {
        	...
        	if(i>0) {
        		...
        		intersec = (ArrayList<String>) intersection(q1.getTableList(),q2.getTableList());
        		commonTables   = intersec.size();
        	}
            queryScoreList.add(commonTables.doubleValue());
        }
