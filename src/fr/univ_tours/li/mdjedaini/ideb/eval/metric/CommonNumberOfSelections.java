...
public class CommonNumberOfSelections extends Metric {

	public CommonNumberOfSelections(BenchmarkEngine arg_be) {
        super(arg_be);
        this.name           = "Metric - Common selections number";
        this.description    = "Computes iteratively the number of common selections between queries";
    }
    ...
    public MetricScore apply(Exploration arg_tr) {
        ...
        for(int i = 0; i < arg_tr.getWorkSession().getNumberOfQueries(); i++) {
        	...
        	if(i>0) {
        		...
        		intersec = (ArrayList<String>) intersection(q1.getSelectionList(),q2.getSelectionList());
        		commonSelections   = intersec.size();
        	}
            queryScoreList.add(commonSelections.doubleValue());
        }
